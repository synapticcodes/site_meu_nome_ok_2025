...
declare
  ...
  v_meta_result net.http_response_result;
  ...
  if new.contact_fingerprint is null then
    ...
    if v_settings.lead_status_url is null or v_settings.secret is null then
      ...
      begin
        ...
        begin
          select net.http_collect_response(v_request_id)
            into v_http_result;

          if v_http_result.response is not null then
          ...
          if v_http_result.status = 'ERROR' then
            v_success := false;
            if v_error is null then
              v_error := coalesce(v_http_result.message, 'unknown error');
            end if;
          else
  ...
  if v_meta_event is not null and v_pixel_id is not null then
    if v_meta_access_token is null then
      ...
      begin
        ...
        begin
          select net.http_collect_response(v_meta_request)
            into v_meta_result;

          if v_meta_result.response is not null then
          ...
          if v_meta_result.status = 'ERROR' then
            v_meta_success := false;
            if v_meta_error is null then
              v_meta_error := coalesce(v_meta_result.message, 'unknown error');
            end if;
          else
