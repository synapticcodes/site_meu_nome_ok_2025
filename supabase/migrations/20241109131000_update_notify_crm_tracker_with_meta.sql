...
begin
  ...
  if v_meta_event is not null and v_pixel_id is not null then
    if v_meta_access_token is null then
      ...
      v_gender_hash := null; -- not available
      ...
      begin
        ...
        into v_meta_response;
        v_meta_status := (v_meta_response ->> 'status')::int;
        v_meta_success := v_meta_status between 200 and 299;
        if not v_meta_success then
          v_meta_error := v_meta_response ->> 'body';
        end if;
      exception when others then
