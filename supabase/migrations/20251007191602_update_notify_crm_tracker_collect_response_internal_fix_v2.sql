...
declare
  ...
  v_http_status net.request_status;
  v_http_message text;
  v_http_response net.http_response;
  v_http_result record;
  -- Meta dispatch
  ...
  v_meta_collect_status net.request_status;
  v_meta_collect_message text;
  v_meta_response net.http_response;
  v_meta_result record;
  v_email_hash text;
  v_phone_digits text;
  v_phone_hash text;
...
begin
  ...
  if new.contact_fingerprint is null then
    ...
    if v_settings.lead_status_url is null or v_settings.secret is null then
      ...
      begin
        ...
        begin
          select (net._http_collect_response(v_request_id, true)).*
            into v_http_result;

          v_http_status := v_http_result.status;
          v_http_message := v_http_result.message;
          v_http_response := v_http_result.response;

          if v_http_response is not null then
            v_status := v_http_response.status_code;
            v_headers := v_http_response.headers;
            v_body := v_http_response.body;
          else
            v_status := null;
            v_headers := null;
            v_body := null;
          ...
          if v_http_status = 'ERROR' then
            v_success := false;
            if v_error is null then
              v_error := coalesce(v_http_message, 'unknown error');
            end if;
          else
            v_success := coalesce(v_status between 200 and 299, false);
          end if;
  ...
  if v_body is not null then
    begin
      v_body_json := v_body::jsonb;
    exception when others then
      v_body_json := jsonb_build_object('raw', v_body);
    end;
  else
    v_body_json := null;
  end if;
  ...
  if v_meta_event is not null and v_pixel_id is not null then
    if v_meta_access_token is null then
      ...
      begin
        ...
        begin
          select (net._http_collect_response(v_meta_request, true)).*
            into v_meta_result;

          v_meta_collect_status := v_meta_result.status;
          v_meta_collect_message := v_meta_result.message;
          v_meta_response := v_meta_result.response;

          if v_meta_response is not null then
            v_meta_status := v_meta_response.status_code;
            v_meta_headers := v_meta_response.headers;
            v_meta_body := v_meta_response.body;
          else
            v_meta_status := null;
            v_meta_headers := null;
            v_meta_body := null;
          ...
          if v_meta_collect_status = 'ERROR' then
            v_meta_success := false;
            if v_meta_error is null then
              v_meta_error := coalesce(v_meta_collect_message, 'unknown error');
            end if;
          else
